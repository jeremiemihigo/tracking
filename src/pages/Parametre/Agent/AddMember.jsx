...
React.useEffect(()=>{
  ...
  return (
    <div style={{ width: '20rem', marginTop: '10px' }}>
      ...
      {loadRegion ? <p style={{fontSize:"12px", textAlign:"center"}}>Loading region...</p> : listeRegion && listeRegion.length > 0 && (
        <div style={{margin:"10px 0px"}}>
        <AutoComplement value={shopRegion} setValue={setShopRegion} options={listeRegion} title="Selectionnez la region" propr="denomination" />
        </div>
        )}
      {loadShop ? <p style={{fontSize:"12px", textAlign:"center"}}>Loading shop...</p> : shopListe && shopListe.length > 0 && (
        <div style={{margin:"10px 0px"}}>
